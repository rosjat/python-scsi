...
class CdbWrite12Test(unittest.TestCase):
    def test_main(self) -> None:
        with MockSCSI(MockDevice(sbc)) as s:
            ...
            raw_cdb = w.cdb
            self.assertEqual(raw_cdb[0], s.device.opcodes.WRITE_12.value)
            self.assertEqual(raw_cdb[1], 0)
            self.assertEqual(scsi_ba_to_int(raw_cdb[2:6]), 1024)
            self.assertEqual(scsi_ba_to_int(raw_cdb[6:10]), 27)
            self.assertEqual(raw_cdb[10], 0)
            self.assertEqual(raw_cdb[11], 0)
            ...
            raw_cdb = w.cdb
            self.assertEqual(raw_cdb[0], s.device.opcodes.WRITE_12.value)
            self.assertEqual(raw_cdb[1], 0x58)
            self.assertEqual(scsi_ba_to_int(raw_cdb[2:6]), 65536)
            self.assertEqual(scsi_ba_to_int(raw_cdb[6:10]), 27)
            self.assertEqual(raw_cdb[10], 0x13)
            self.assertEqual(raw_cdb[11], 0)
            cdb = w.unmarshall_cdb(raw_cdb)
            self.assertEqual(cdb["opcode"], s.device.opcodes.WRITE_12.value)

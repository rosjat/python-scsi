...
class UnmarshallModesense10Test(unittest.TestCase):
    def test_main(self) -> None:
        ...
        with MockSCSI(MockModeSenseEAA(smc)) as s:
            ...
            self.assertEqual(
                i["mode_pages"][0]["page_code"],
                MODESENSE10.PAGE_CODE.ELEMENT_ADDRESS_ASSIGNMENT,
            )
            self.assertEqual(
                i["mode_pages"][0]["first_medium_transport_element_address"], 257
            )
            ...
            self.assertEqual(i["mode_pages"][0]["spf"], 0)
            self.assertEqual(
                i["mode_pages"][0]["page_code"], MODESENSE10.PAGE_CODE.CONTROL
            )
            ...
            i = s.modesense10(
                page_code=MODESENSE10.PAGE_CODE.CONTROL, sub_page_code=1
            ).result
            self.assertEqual(i["medium_type"], 0)
            self.assertEqual(i["device_specific_parameter"], 0x90)

...
class CdbPreventallowMediumremovalTest(unittest.TestCase):
    def test_main(self) -> None:
        with MockSCSI(MockDevice(smc)) as s:
            ...
            raw_cdb = m.cdb
            self.assertEqual(
                raw_cdb[0], s.device.opcodes.PREVENT_ALLOW_MEDIUM_REMOVAL.value
            )
            self.assertEqual(raw_cdb[4], 0x03)
            cdb = m.unmarshall_cdb(raw_cdb)
            self.assertEqual(
                cdb["opcode"], s.device.opcodes.PREVENT_ALLOW_MEDIUM_REMOVAL.value
            )

...
class MockInquiryStandard(MockDevice):
    def execute(self, cmd: SCSICommand[Any], en_raw_sense: bool = False) -> None:
        ...
        # t10 vendor id
        cmd.datain[8:16] = bytearray(
            ord(c) for c in ["a", "b", "c", "d", "e", "f", "g", "h"]
        )
        # product id
        cmd.datain[16:32] = bytearray(
            ord(c)
            for c in [
                "i",
                "i",
...
class UnmarshallInquiryTest(unittest.TestCase):
    def test_main(self) -> None:
        with MockSCSI(MockInquiryStandard(sbc)) as s:
            ...
            self.assertEqual(
                i["product_identification"].decode("utf-8"), "iiiiiiiijjjjjjjj"
            )
        ...
        with MockSCSI(MockLBP(sbc)) as s:
            ...
            self.assertEqual(
                i["provisioning_type"], INQUIRY.PROVISIONING_TYPE.THIN_PROVISIONED
            )
        ...
        with MockSCSI(MockExtendedInquiry(sbc)) as s:
            ...
            self.assertEqual(
                dd[0]["designator"]["t10_vendor_id"].decode("utf-8"), "Test T10"
            )
            self.assertEqual(
                dd[0]["designator"]["vendor_specific_id"].decode("utf-8"), ""
            )
            ...
            self.assertEqual(
                dd[1]["designator"]["vendor_specific_extension_id"].decode("utf-8"),
                "abcde",
            )
